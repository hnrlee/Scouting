...
public class WebApi
{
	...
	public void getMatchDetails(String event, int teamNumber, String level)
	{
		...
		{
			Client client = ClientBuilder.newClient();
			Response response = client.target("https://frc-api.firstinspires.org/v2.0/2016/scores/" + event + "/" + level + "?teamNumber=" + teamNumber).request(MediaType.TEXT_PLAIN_TYPE).header("Authorization", apiKey).get();
			System.out.println("status: " + response.getStatus());
			System.out.println("headers: " + response.getHeaders());
			String body = response.readEntity(String.class);
			try
